...
for i in range(num_epochs):
    #np.random.shuffle(train_X)        #shuffle batch groups
                                      #shuffle labels
    train_X, train_y = skShuffle(train_X, train_y)

    t = i * batch_size
    train_function(inputs=(train_X[t: t + batch_size,:], train_y[t: t + batch_size][:, np.newaxis]))
